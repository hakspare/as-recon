...
install_pkg() {
    ...
    if ! command -v "$PKG" &>/dev/null; then
        ...
        elif command -v apt &>/dev/null; then sudo apt update && sudo apt install -y "$PKG"
        ...
        else echo -e "${RED}✗ Cannot install ${PKG}, install manually.${NC}"; fi
    else
...
# ----------------------------
# Clone / Update Repo (Single Folder)
# ----------------------------
...
if [ -f "pyproject.toml" ]; then
    ...
    echo -e "${RED}✗ pyproject.toml not found! Ensure you are in repo root.${NC}"
fi

# ----------------------------
# Global install AS-RECON via pipx
# ----------------------------
echo -e "${BLUE}→ Installing AS-RECON globally...${NC}"
pipx install . --force || echo -e "${RED}pipx install failed. Run manually: pipx install .${NC}"
...
if [ -f "$API_FILE" ]; then
    if command -v jq &>/dev/null && ! jq . "$API_FILE" >/dev/null 2>&1; then
        echo -e "${YELLOW}⚠️ ${API_FILE} invalid JSON${NC}"
    else
        echo -e "${GREEN}✓ API keys detected and valid${NC}"
    fi
else
